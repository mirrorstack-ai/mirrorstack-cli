#![allow(clippy::field_reassign_with_default)] // snapshots are built field by field
...
fn roles_approvals_and_unmapped_roles() {
    ...
    assert_eq!(
        plan.rows["user_approval"][0].data["categoryKey"],
        "membership"
    );
    let ex: Vec<_> = plan
        .exceptions
...
fn refused_media_makes_the_video_and_its_rows_exceptions() {
    ...
    let media = [(
        "v".to_string(),
        MediaOutcome::Refused(Refusal::Plaintext("720p".into())),
    )]
    .into();
    let m = maps();
    let mut plan = Plan::default();
    ...
    let ex: Vec<_> = plan
        .exceptions
        .iter()
        .map(|e| (e.entity.as_str(), e.reason.as_str()))
        .collect();
